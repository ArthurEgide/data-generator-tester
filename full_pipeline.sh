...
docker run --network tcc_egide --memory="1g" --cpus="2.0" -it --rm -v ./generators/faker-js_faker/:/generator generator_fakerjs run

# Set up Fakedata
docker build -t generator_fakedata generators/lucapette-fakedata/
docker run --network tcc_egide --memory="1g" --cpus="2.0" -it --rm -e PGPASSWORD=egide -v ./generators/lucapette-fakedata/:/generator generator_fakedata run